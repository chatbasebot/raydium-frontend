...
import { Liquidity, struct, u64 } from "raydium-sdk-devnet";
...
export default function CreatePoolPage() {
  return (
    <PageLayout
      metaTitle="Create Liquidity Pool - Raydium"
      mobileBarTitle="Create Pool"
    >
...
function PanelContent({ close }: { close(): void }) {
  ...
  const step1Content = (
    <>
      ...
      <div className="my-12 mobile:my-6 italic text-center text-sm font-medium text-[rgba(171,196,255,.5)]">
        This tool is for advanced users. Before attempting to create a new
        liquidity pool, we suggest going through this{" "}
        <Link href="https://raydium.gitbook.io/raydium/permissionless/creating-a-pool">
          detailed guide
        </Link>
      </div>
      <InputBox
  ...
  const baseToken = baseMint
    ? getToken(baseMint) ??
      (baseDecimals != null
        ? createSplToken({ mint: baseMint, decimals: baseDecimals })
        : undefined)
    : undefined;
  const quoteToken = quoteMint
    ? getToken(quoteMint) ??
      (quoteDecimals != null
        ? createSplToken({ mint: quoteMint, decimals: quoteDecimals })
        : undefined)
    : undefined;
  ...
  const haveEnoughCoinBase =
    baseAmount && baseTokenBalance && lte(baseAmount, baseTokenBalance);
  const haveEnoughCoinQuote =
    quoteAmount && quoteTokenBalance && lte(quoteAmount, quoteTokenBalance);
  ...
  const step2Content = (
    <>
      ...
      <Col className="my-12 mobile:my-6 py-4 px-6 flex-grow ring-inset ring-1.5 ring-[rgba(171,196,255,.5)] rounded-xl items-center gap-3 mobile:gap-1">
        <AddressItem
          fieldName="OpenBook Market ID:"
          fieldValue={marketId ?? "--"}
        />
        <AddressItem
          fieldName="Base Token Mint Address:"
          fieldValue={baseMint ?? "--"}
          autoShowTokenSymbol
        />
        <AddressItem
          fieldName="Quote Token Mint Address:"
          fieldValue={quoteMint ?? "--"}
          autoShowTokenSymbol
        />
      ...
      {baseAmount &&
        quoteAmount &&
        isMeaningfulNumber(baseAmount) &&
        isMeaningfulNumber(quoteAmount) && (
      ...
      <CoinInputBox
        ...
        onUserInput={(inputText) =>
          useCreatePool.setState({ baseDecimaledAmount: inputText })
        }
      />
      <CoinInputBox
        ...
        onUserInput={(inputText) =>
          useCreatePool.setState({ quoteDecimaledAmount: inputText })
        }
      />
      <DateInput
        className="mb-5"
        ...
        onDateChange={(selectedDate) =>
          useCreatePool.setState({ startTime: selectedDate })
        }
        ...
        <Tooltip>
          ...
          <Tooltip.Panel>
            <div className="max-w-[300px] space-y-1.5">
              Pool creation fee ensures lookup table support for optimized pool
              indexing, swap routing, and API functionality.
            </div>
          </Tooltip.Panel>
        </Tooltip>
      </Row>
      ...
      <Button
        ...
        validators={[
          ...
          { should: Boolean(baseAmount && quoteAmount) },
          {
            ...
            fallbackProps: {
              children: `Insufficient ${baseToken?.symbol ?? ""} balance`,
            },
          },
          {
            should: haveEnoughCoinQuote,
            fallbackProps: {
              children: `Insufficient ${quoteToken?.symbol ?? ""} balance`,
            },
          },
        ]}
        onClick={() => {
  ...
  const step3Content = (
    <>
      ...
      <div className="font-medium text-center">
        Pool has been successfully created!
      </div>
        ...
        <InfoItem
          ...
          fieldValue={
            <Row>
              <Link
                className="text-sm"
                href={`/liquidity/add/?ammId=${ammId}`}
                onClick={close}
              >
  ...
  return (
    <Row className={twMerge("w-full justify-between", className)}>
      ...
      <div className="text-xs font-medium text-white mobile:text-2xs">
        {autoShowTokenSymbol
          ? mintSearch(String(fieldValue)) ?? fieldValue
          : fieldValue}
      </div>
    </Row>
  );
...
function AddressItem({
  ...
  const shortAddress = useMemo(
    () => fieldValue.slice(0, 6) + "......" + fieldValue.slice(-6),
    [fieldValue]
  );
  return (
    <InfoItem
      ...
      fieldValue={
        <Row>
          <div title={fieldValue}>
            {shortAddress}{" "}
            {autoShowTokenSymbol && mintSearch(String(fieldValue))}
          </div>
          <Icon
            size="sm"
            heroIconName="clipboard-copy"
...
function UserCreatedPoolsExhibitionPanel() {
  ...
  const userExhibitionHistory = useMemo(
    () => owner && createdPoolHistory?.[String(owner)],
    [createdPoolHistory, owner]
  );
  ...
  return (
    <div className="self-center w-[min(456px,90vw)] mt-12">
      <div className="pb-8 text-xl mobile:text-base font-semibold justify-self-start text-white">
        Your Created Pool
      </div>
            ...
            >
              ...
              <Collapse.Body>
                <Col className="border-t-1.5 border-[rgba(171,196,255,.5)] mt-4 py-5 gap-3">
                  ...
                  <AddressItem
                    fieldName="Market Id: "
                    fieldValue={info.marketId}
                  />
                  <AddressItem
                    fieldName="Base Mint: "
                    fieldValue={info.baseMint}
                    autoShowTokenSymbol
                  />
                  <AddressItem
                    fieldName="Quote Mint: "
                    fieldValue={info.quoteMint}
                    autoShowTokenSymbol
                  />
                  <InfoItem
                    fieldName="Created On: "
                    fieldValue={info.timestamp}
                  />
                </Col>
...
function useCreatePoolSOLCost(): number | undefined {
  ...
  const cost = useAsyncMemo(async () => {
    ...
    try {
      return (
        (
          await Liquidity.getCreatePoolFee({
            connection,
